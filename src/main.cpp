...
#define IN1   25   // D25 → L298N IN1
#define IN2   33   // D33 → L298N IN2
#define ENA   32   // D32 → L298N ENA (PWM)

#define IN3   26   // D26 → L298N IN3
#define IN4   27   // D27 → L298N IN4
#define ENB   14   // D14 → L298N ENB (PWM)
...
// ==================== SERVO ====================
const int SERVO_STEP_DELAY = 7;  // ms giữa mỗi bước 1° (càng lớn càng chậm)

// Hàm ghi trực tiếp duty cho servo (không smooth, dùng khi init)
void writeServo(uint8_t channel, int angle) {
  int duty = map(constrain(angle, 0, 180), 0, 180, 1638, 8192);
  ledcWrite(channel, duty);
}

// Di chuyển servo từ từ từ currentPos đến targetAngle, bước 1°
void moveServoSmooth(uint8_t channel, int &currentPos, int targetAngle) {
  targetAngle = constrain(targetAngle, 0, 180);
  float step = (targetAngle > currentPos) ? 1 : -1;
...
void setLiftLeft(int angle) {
  moveServoSmooth(SERVO_CH_LIFT_L, liftLeftPosition, angle);
}

void setLiftRight(int angle) {
  moveServoSmooth(SERVO_CH_LIFT_R, liftRightPosition, angle);
}

void setGripLeft(int angle) {
  moveServoSmooth(SERVO_CH_GRIP_L, gripLeftPosition, angle);
}

void setGripRight(int angle) {
  moveServoSmooth(SERVO_CH_GRIP_R, gripRightPosition, angle);
}
...
void setup() {
  ...
  // GPIO động cơ (L298N direction pins)
  pinMode(IN1, OUTPUT);
  pinMode(IN2, OUTPUT);
  pinMode(IN3, OUTPUT);
  pinMode(IN4, OUTPUT);
  ...
  // Vị trí mặc định servo (ghi trực tiếp, không smooth lúc khởi tạo)
  writeServo(SERVO_CH_LIFT_L, goc_ha);
  writeServo(SERVO_CH_LIFT_R, goc_ha);
  writeServo(SERVO_CH_GRIP_L, goc_mo_cang);
  writeServo(SERVO_CH_GRIP_R, goc_mo_cang);
  liftLeftPosition  = goc_ha;
  liftRightPosition = goc_ha;
  gripLeftPosition  = goc_mo_cang;
  gripRightPosition = goc_mo_cang;
